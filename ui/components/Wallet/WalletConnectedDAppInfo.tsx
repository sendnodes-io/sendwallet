...
export default function (props: {
  ...
  useEffect(() => {
    const filtered = Object.keys(unfiltered).filter(o => o !== pKey).map(k => unfiltered[k])
    const perms = (pKey && unfiltered[pKey]) ? [currentPermission, ...filtered] : filtered;
    ...
    <div className="wrap">
      <ul>
        { permissions.map((p,i) => {
          ...
          return (
            <li key={`${o}-${i}`} className={`dApp ${k === pKey ? 'active' : 'inactive dashed_border_thin'}`}>
              <div className="left">
              ...
              <div className="right">
                <button onClick={() => { dispatch(denyOrRevokePermission({...p, state: "deny"}))}}><div className={`icon disconnect`}/></button>
              </div>
            </li>
          )
        })}
