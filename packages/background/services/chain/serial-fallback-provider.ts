...
import { EVMAddressOnNetwork } from "../../accounts";
...
export default class SerialFallbackProvider extends JsonRpcProvider {
	...
	async send(method: string, params: unknown): Promise<unknown> {
		try {
			...
			return await this.currentProvider.send(method, params as unknown[]);
		} catch (error) {
	...
	async subscribeFullPendingTransactions(
		{ address, network }: EVMAddressOnNetwork,
		handler: (pendingTransaction: AnyEVMTransaction) => void,
	): Promise<void> {
		if (this.evmNetwork.chainID !== network.chainID) {
			logger.error(
				`Tried to subscribe to pending transactions for chain id ${network.chainID} but provider was on ${this.evmNetwork.chainID}`,
			);
			return;
		}
			...
			await this.alchemySubscribeFullPendingTransactions(
				...
				if (
					e
						.toString()
						.match(
							/Invalid value "alchemy_filteredNewFullPendingTransactions" supplied to/,
						)
				) {
					return "unsupported";
				}
	...
	private async resubscribe(provider: JsonRpcProvider): Promise<boolean> {
		...
		if (!(provider instanceof WebSocketProvider)) {
			if (this.subscriptions.length > 0) {
				logger.warn(
					`Cannot resubscribe ${this.subscriptions.length} subscription(s) as the current provider is not a WebSocket provider; waiting until a WebSocket provider connects to restore subscriptions properly.`,
				);
