...
export default class PocketProvider extends BaseService<ServiceLifecycleEvents> {
  ...
  async poll(): Promise<void> {
    ...
    this._events.forEach((event) => {
      switch (event.type) {
        case "tx": {
          const { hash } = event
          const runner: Promise<any> = this.getTransaction(hash)
            .then((tx) => {
              if (!tx || !tx.height || tx.height.toString() == "0") return null
  ...
  async sendTransaction(
    ...
    const result = await this.rpc.client.rawtx(transaction.from, transaction.tx)
  ...
  async getTransaction(transactionHash: string): Promise<Transaction> {
    return poll(
      async () => {
        const result = (await this.rpc.query.getTX(transactionHash)) as any
        if (result instanceof RpcError) {
          // if (this._emitted[`t:${transactionHash}`] == null) {
          //   console.log("getTx return null")
          //   return null
  ...
  async getSkinnyBlock(blockNumber: number): Promise<POKTWatchBlock> {
    return poll(
      async () => {
