...
const accountSlice = createSlice({
  ...
  reducers: {
    ...
    /**
     * Reset all accounts data to loading useful for when swtiching networks.
     */
    resetAccountsData: (state) => {
      return {
        ...state,
        accountsData: Object.fromEntries(Object.keys(state.accountsData).map((k) => [k, "loading"])),
      }
    },
    loadAccount: (
      state,
