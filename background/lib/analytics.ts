...
const ANALYTICS_PATH = process.env.NODE_ENV === "development" ?
...
async function postAnalytics(gaParams: URLSearchParams) {
  try {
    gaParams.set('z', Math.random().toString() + new Date().getTime().toString())
    ...
    if (process.env.NODE_ENV === "development") {
      console.debug("Sent analytics", { gaParams: Object.fromEntries(gaParams as unknown as Iterable<any>), response })
    }
  } catch (e) {
    if (process.env.NODE_ENV === "development") {
